...
export default class extends React.Component {
  ...
  componentDidMount() {
    this.setState({ assets: this.sort(this.props.assets) });
  }

  componentWillReceiveProps(nextProps) {
    this.setState({ assets: this.sort(nextProps.assets) });
    if (nextProps.anchor) {
  ...
  sort(assets) {
    const slides = [];
    let count = -1;
    assets.filter(asset => !asset.inactive).forEach((asset, i) => {
      if (i % 6 === 0) {
        count++;
        slides[count] = [];
      }
      slides[count].push(asset);
    });
    return slides;
  }
  ...
  trackRedirect = assetId => {
    ReactGA.event({
      category: 'Map asset redirect',
      action: 'Map asset redirect',
  ...
  render() {
    ...
    return (
      <Section id="list">
        <Header>
          <Heading>Marketplace Assets</Heading>
          <Tagline>Click below to view and purchase asset data stream</Tagline>
        </Header>
        <SlideWrapper index={slideIndex}>
          {Object.keys(assets).map((company, i) => (
            <Slide index={i} slide={slideIndex} key={`company-${i}`}>
              {assets[company].map((item, index) => (
                <Card
                  data-component="AssetCard"
                  key={`asset-${index}`}
                  to={`/asset/${item.assetId}`}
                  onClick={() => this.trackRedirect(item.assetId)}
                >
                  <CardHeader>
                    <AssetType>{item.type}</AssetType>
                    <AssetId>
                      {item.assetId && item.assetId.length > 20
                        ? `${item.assetId.substr(0, 20)}...`
                        : item.assetId}
                    </AssetId>
                  </CardHeader>
