...
const getConfigFileContent = (asset, provider) => `{
...
export const getZip = async (asset, provider) => {
  const zip = new JSZip();
  zip.file('config.json', getConfigFileContent(asset, provider));
  ...
  zip
    ...
    .then(
      blob => FileSaver.saveAs(blob, asset.assetId + '-template.zip'),
