...
class Asset extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (typeof this.props.asset === 'string') {
      ReactGA.event({
        category: 'Purchase failure, No asset',
        action: 'Purchase failure, No asset',
        label: `Asset ID ${assetId}`
      });
      return this.setNotification('noAsset');
    }
  ...
  async purchase() {
    const { loadUser, user, asset,  match: { params: { assetId } } } = this.props;
    ...
    if (Number(wallet.balance) < Number(asset.price)) {
      ReactGA.event({
    ...
    purchaseStream(userId, assetId)
      ...
      .catch(error => {
        ReactGA.event({
          ...
          label: `Asset ID ${assetId}, user ID ${userId}, error: ${error}`
        });
  ...
  render() {
    ...
    return (
      <Main>
        <Cookie />
        <AssetContext.Provider value={{ userId, setErrorState: this.setErrorState, setNotification: this.setNotification }}>
          <AssetNav />
        ...
        <Data>
          ...
          />
          <AssetContext.Provider value={{ func: this.loadMore }}>
            <DataStream packets={packets} streamLength={streamLength} />
          </AssetContext.Provider>
        </Data>
        <Modal
          purchasePrice={this.props.asset.price}
